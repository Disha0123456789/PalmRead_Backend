...
# ### Data Loading and Rectification ###

# # Rectify images in PLSU folder and apply homography matrix
# def rectify(idx):
#     img_path = './PLSU/PLSU/'
#     image = cv2.imread(img_path + f'img/image{idx}.jpg')
#     image_mask = cv2.imread(img_path + f'Mask/image{idx}.png', cv2.IMREAD_GRAYSCALE)
#     mp_hands = mp.solutions.hands

#     pts_index = list(range(21))
#     pts_target_normalized = np.float32([[1-0.482, 0.906], [1-0.604, 0.811], [1-0.676, 0.679],
#                                         [1-0.734, 0.571], [1-0.789, 0.509], [1-0.565, 0.511],
#                                         [1-0.597, 0.365], [1-0.613, 0.271], [1-0.619, 0.192],
#                                         [1-0.492, 0.498], [1-0.489, 0.321], [1-0.489, 0.212],
#                                         [1-0.483, 0.129], [1-0.425, 0.518], [1-0.403, 0.358],
#                                         [1-0.393, 0.261], [1-0.386, 0.177], [1-0.363, 0.564],
#                                         [1-0.335, 0.447], [1-0.320, 0.374], [1-0.312, 0.302]])
...
#         image_height, image_width, _ = image.shape
#         hand_landmarks = results.multi_hand_landmarks[0]
#         pts = np.float32([[landmark.x * image_width, landmark.y * image_height] for landmark in hand_landmarks.landmark])
#         pts_target = np.float32([[x * image_width, y * image_height] for x, y in pts_target_normalized])
...
# # Load rectified images from PLSU folder
# def load_data(num_data):
#     data_path = './line_sample'
#     result_path = './line_sample_result'
...
#     cur_num_data = len(os.listdir(data_path))
#     offset = 50
#     img_path_list = glob.glob('./PLSU/PLSU/Img/*')[offset + cur_num_data:offset + num_data]
...
#             continue
#         cv2.imwrite(f'line_sample/image{i + cur_num_data}.png', rectified_image)

# ### Graph Backtracking and Line Detection ###

# # Optimized backtracking through nodes
# def backtrack(lines_node, temp, graph, visited_node, finished_node, node):
#     end_pt = True
#     for next_node in graph[node].keys():
#         if not visited_node[next_node]:
...
#             visited_node[next_node] = False
#     if end_pt:
#         lines_node.append(temp[:])

# # Group function to identify lines by building and backtracking graph
# def group(img):
#     count = np.zeros(img.shape)
#     nodes = []

#     # Optimized loop: Remove unnecessary check for all non-zero points
#     for j in range(1, img.shape[0] - 1):
#         for i in range(1, img.shape[1] - 1):
#             if img[j, i] == 0:
#                 continue
...
#     # Create graph without unnecessary complexity
#     for node in nodes:
#         y, x = node
#         not_visited[y, x] = 0
#         around = np.multiply(count[y-1:y+2, x-1:x+2], not_visited[y-1:y+2, x-1:x+2])
...
#         for dy, dx in next_pos:
#             y, x = node
#             next_y, next_x = y + dy - 1, x + dx - 1
#             temp_line = [[y, x, 0, 0], [next_y, next_x, dy-1, dx-1]]
#             if count[next_y, next_x] == 1 or count[next_y, next_x] >= 3:
#                 not_visited[next_y, next_x] = 1
#                 graph[tuple(temp_line[0][:2])][tuple(temp_line[-1][:2])] = temp_line
#                 graph[tuple(temp_line[-1][:2])][tuple(temp_line[0][:2])] = temp_line[::-1]
#                 continue
...
#     # Perform backtracking for each unvisited node
#     for node in nodes:
#         if not finished_node[node]:
#             temp = [node]
#             visited_node[node] = True
...
#     # Filter and return lines with more than 10 points
#     lines = []
#     for line_node in lines_node:
#         if len(line_node) > 1:
#             line = []
#             for i in range(len(line_node) - 1):
#                 line.extend(graph[line_node[i]][line_node[i+1]])
#             if len(line) >= 10:
#                 lines.append(line)
    
#     return lines

# ### Select Three Lines Based on Clustering ###

# # Classify lines and select three nearest clusters using KMeans
# def select_three_lines(lines):
#     features = np.array([np.array(line).flatten()[:10] for line in lines])  # feature extraction (simplified)
#     kmeans = KMeans(n_clusters=3, random_state=0).fit(features)
#     cluster_centers = kmeans.cluster_centers_
    
#     # Select the closest line to each cluster center
#     selected_lines = []
#     for center in cluster_centers:
#         distances = np.linalg.norm(features - center, axis=1)
#         nearest_line_idx = np.argmin(distances)
#         selected_lines.append(lines[nearest_line_idx])
...
#         img = cv2.imread(f'line_sample/image{i}.png', cv2.IMREAD_GRAYSCALE)
#         lines = group(img)
#         selected_lines = select_three_lines(lines)
#         print(f"Selected Lines {i}: ", selected_lines)
