...
def warp_image(path_to_image, path_to_warped_image):
    # 7 key landmark points (normalized coordinates)
    pts_index = list(range(21))
    ...
    with mp_hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.5) as hands:
        ...
        if image is None:
            print(f"Error: Unable to load image from {path_to_image}")
            return None
        ...
        # Extract landmark points and compute homography
        hand_landmarks = results.multi_hand_landmarks[0]
        pts = np.float32([
            [hand_landmarks.landmark[i].x * image_width, hand_landmarks.landmark[i].y * image_height]
            for i in pts_index[:7]  # Limit to essential 7 points for faster homography calculation
        ])
...
def warp(path_to_input_image, path_to_warped_image):
    # Handle HEIC images or convert if necessary
    if path_to_input_image.lower().endswith('heic'):
        try:
            import pyheif
            heif_file = pyheif.read(path_to_input_image)
            ...
            path_to_input_image = path_to_input_image[:-5] + '.jpg'
            image.save(path_to_input_image, "JPEG")
        except ImportError:
            print("Please install pyheif to process HEIC images.")
            return None

    # Perform image warping
    warp_result = warp_image(path_to_input_image, path_to_warped_image)
    return warp_result if warp_result else None
