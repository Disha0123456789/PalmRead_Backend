...
# class DoubleConv(nn.Module):
#     """(convolution => [BN] => ReLU) * 2, optimized with separable convolutions"""
#     def __init__(self, in_channels, out_channels, mid_channels=None):
#         super().__init__()
#         if not mid_channels:
#             mid_channels = out_channels
...
#     def forward(self, x1, x2):
#         x1 = self.up(x1)
#         # padding to match sizes for concatenation
#         diffY = x2.size()[2] - x1.size()[2]
#         diffX = x2.size()[3] - x1.size()[3]
#         x1 = F.pad(x1, [diffX // 2, diffX - diffX // 2,
#                         diffY // 2, diffY - diffY // 2])
...
# class ContextFusion(nn.Module):
#     """Modified ContextFusion for faster computation"""
#     def __init__(self, channels):
#         super().__init__()
#         self.context_modeling = nn.Conv2d(channels, channels, kernel_size=1)  # Simplified
#         self.context_transform1 = nn.Conv2d(channels, channels, kernel_size=1)
#         self.context_transform2 = nn.Conv2d(channels, channels, kernel_size=1)

#     def forward(self, x):
#         x1 = nn.MaxPool2d(2)(x)
#         x2 = self.context_modeling(x1)
#         return self.context_transform1(x2) * x1 + self.context_transform2(x2)
